...
async function loadLatestManifest() {
  ...
  try {
    const response = await fetch("/public/v0.2/latest.json", { cache: "no-store" });
    if (!response.ok) {
...
async function loadServerManifest() {
  ...
  try {
    ...
    serverEl.dataset.status = "offline";
    serverEl.title = "Server status is temporarily unavailable.";
  }
}
...
function getEffectiveServerStatus(manifest, now = new Date()) {
  ...
  const normalizedUpdatedAt = String(manifest.updatedAt || "").replace(
    /(\.\d{3})\d+(Z|[+-]\d{2}:\d{2})$/,
    "$1$2"
  );
  const updatedAt = new Date(normalizedUpdatedAt);
